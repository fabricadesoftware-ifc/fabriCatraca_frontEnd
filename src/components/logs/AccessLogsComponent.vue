  ...
  <div>
    ...
    <v-card class="mb-4">
      <v-card-text>
        <v-row align="center">
          ...
          <v-col cols="12" md="2">
            <v-btn block color="primary" prepend-icon="mdi-magnify" @click="onSearch">
              Buscar
            </v-btn>
          </v-col>
          <v-col cols="12" md="2">
      ...
      >
        <template #item.time="{ item }">
          <div class="d-flex align-center">
            ...
            <span class="text-body-2">{{ formatDateTime(String(item.time || "")) }}</span>
          </div>
        </template>
        ...
        <template #item.user="{ item }">
          <div class="d-flex align-center">
            <v-avatar class="mr-2" color="primary" size="32">
              <span class="text-caption">{{ getUserName(item).charAt(0).toUpperCase() }}</span>
            </v-avatar>
            <div>
              <div class="font-weight-medium">
                {{ getUserName(item) }}
              </div>
              <div v-if="item.user_id" class="text-caption text-grey">ID: {{ item.user_id }}</div>
            </div>
          </div>
        </template>
        ...
        <template #item.device="{ item }">
          <div class="d-flex align-center">
            <v-icon class="mr-2" color="grey" size="small"> mdi-turnstile </v-icon>
            {{ getDeviceName(item) }}
          </div>
        </template>
        ...
        <template #bottom>
          ...
          <div class="d-flex align-center justify-space-between pa-4">
            ...
            <div class="text-body-2">Mostrando {{ logs.length }} de {{ totalItems }} registros</div>
            <v-pagination
              v-model="currentPage"
              :length="totalPages"
              :total-visible="5"
        ...
        <template #no-data>
          <div class="text-center pa-8">
            <v-icon class="mb-4" color="grey" size="64"> mdi-clipboard-text-off </v-icon>
            <p class="text-h6 text-grey">Nenhum log encontrado</p>
            <p class="text-body-2 text-grey">Tente ajustar os filtros ou aguarde novos registros</p>
          </div>
        </template>
      </v-data-table>
    </v-card>
    ...
    <v-dialog v-model="detailDialog" max-width="600">
      <v-card v-if="selectedLog">
        ...
        <v-card-text>
          <v-list>
            ...
            <v-list-item>
              ...
              <v-list-item-subtitle>{{ getUserName(selectedLog) }}</v-list-item-subtitle>
            </v-list-item>
            ...
            <v-list-item v-if="selectedLog.user_id">
              ...
              <v-list-item-subtitle>{{ selectedLog.user_id }}</v-list-item-subtitle>
            </v-list-item>

            <v-list-item>
              <template #prepend>
                <v-icon :color="logStore.getEventTypeColor(Number(selectedLog.event_type))">
              ...
              <v-list-item-subtitle>
                <v-chip
                  :color="logStore.getEventTypeColor(Number(selectedLog.event_type))"
                  size="small"
                >
            ...
            <v-list-item>
              ...
              <v-list-item-subtitle>{{ getDeviceName(selectedLog) }}</v-list-item-subtitle>
            </v-list-item>
            ...
            <v-list-item>
              ...
              <v-list-item-subtitle>{{ getPortalName(selectedLog) }}</v-list-item-subtitle>
            </v-list-item>
            ...
            <v-list-item v-if="selectedLog.portal_id">
              ...
              <v-list-item-subtitle>{{ selectedLog.portal_id }}</v-list-item-subtitle>
            </v-list-item>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn color="primary" variant="text" @click="detailDialog = false"> Fechar </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
